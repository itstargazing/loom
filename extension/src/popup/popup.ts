...
import {
  CAPTURE_EVENT_DESCRIPTIONS,
  CAPTURE_EVENT_LABELS,
  CAPTURE_EVENT_TYPES,
  type CaptureEvent,
...
import {
  loadPrivacySettings,
  matchesLocalDomain,
  refreshPrivacySettingsFromApi,
  saveLocalOnlyDomains,
} from "../capture/privacy-settings";
...
function setBadge(id: string, variant: "success" | "warning" | "error", text: string): void {
...
function renderSignalToggles(enabled: Record<CaptureEventType, boolean>): void {
...
function renderSyncStatus(status: SyncStatus): void {
  ...
  setText(
    "sync-last",
    status.lastSyncedAtMs ? new Date(status.lastSyncedAtMs).toLocaleTimeString() : "never",
  );
...
async function pollSyncStatus(): Promise<void> {
  ...
  if (response?.ok) renderSyncStatus(response.status as SyncStatus);
}

function initSyncSection(): void {
...
async function renderLocalModeSection(): Promise<void> {
...
function initLocalModeSection(): void {
...
async function watchCurrentTab(): Promise<void> {
  const status = document.getElementById("watch-status");
  const [activeTab] = await chrome.tabs.query({ active: true, currentWindow: true });
  const url = activeTab?.url?.trim();
  if (!url || (!url.startsWith("http://") && !url.startsWith("https://"))) {
    if (status) status.textContent = "Open an http(s) page first.";
    return;
  }
  ...
  try {
    ...
    const sets = (await setsResponse.json()) as Array<{ id: string; name: string }>;
    let setId = sets[0]?.id;
    if (!setId) {
      ...
      if (!created.ok) throw new Error(`Create set failed (${created.status})`);
      const body = (await created.json()) as { id: string };
      setId = body.id;
    }

    const added = await fetch(
      `${config.apiBaseUrl}/api/skills/live-doc-diff/sets/${setId}/documents`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${config.authToken}`,
...
function initWatchSection(): void {
...
async function renderPdfStatus(): Promise<void> {
  const [activeTab] = await chrome.tabs.query({ active: true, currentWindow: true });
...
function summarize(event: CaptureEvent): string {
  switch (event.type) {
...
function renderFeed(feed: CaptureFeed): void {
...
async function pollFeed(): Promise<void> {
  ...
  if (response?.ok) renderFeed(response.feed as CaptureFeed);
}
