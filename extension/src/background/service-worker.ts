...
import type { PdfDocumentSnapshot } from "../pdf/types";
 ...
 */
async function capturePdfAsPageOpened(snapshot: PdfDocumentSnapshot): Promise<void> {
  if (snapshot.usingFallback) return;

  const settings = await loadCaptureSettings();
  if (!settings.signals.page_opened) return;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (message?.type) {
    ...
    case "capture:events": {
      const events = (message.events ?? []) as CaptureEvent[];
    ...
    case "pdf:loaded": {
      ...
      console.info(
        `[LOOM] PDF loaded: ${snapshot.title} — ${snapshot.numPages} pages, ` +
    ...
    case "pdf:get-snapshot": {
      ...
      sendResponse(
        snapshot ? { ok: true, snapshot } : { ok: false, error: "No PDF snapshot available" },
      );
      return true;
    }
