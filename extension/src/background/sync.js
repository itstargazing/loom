...
import { enqueue, peekBatch, queueLength, readSyncState, removeEvents, updateSyncState, } from "./sync-queue";
...
function backoffFor(failureCount) {
    const exponential = Math.min(BASE_BACKOFF_MS * 2 ** Math.max(0, failureCount - 1), MAX_BACKOFF_MS);
...
function onFailure(error, retryInMs) {
    return (state) => ({
        ...state,
    ...
    try {
        for (let batchIndex = 0; batchIndex < MAX_BATCHES_PER_RUN; batchIndex += 1) {
            ...
            if (result.permanent) {
                ...
                await removeEvents(new Set(batch.map((event) => event.id)));
                console.error(`[LOOM] Dropped ${batch.length} event(s) rejected by the backend: ${result.error}`);
            ...
            console.warn(`[LOOM] Sync (${reason}) failed: ${result.error}. Retrying in ${Math.round(retryInMs / 1000)}s.`);
...
export async function queueForSync(events) {
    if (events.length === 0)
        return;
