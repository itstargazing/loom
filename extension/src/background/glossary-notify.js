...
async function fetchClassification(apiBaseUrl, authToken, eventId) {
...
function fieldOf(row, category, field) {
...
async function waitForField(apiBaseUrl, authToken, eventId, category, field) {
...
export async function notifyClassifiedSkills(events, tabId) {
    if (tabId === undefined)
        return;
    ...
    if (highlights.length === 0 && copies.length === 0)
        return;
    const config = await loadSyncConfig();
    for (const event of highlights) {
        const term = await waitForField(config.apiBaseUrl, config.authToken, event.id, "glossary_term", "term");
        if (term) {
    ...
    for (const event of copies) {
        const quote = await waitForField(config.apiBaseUrl, config.authToken, event.id, "citation", "quote");
        if (quote) {
            await notifyTab(tabId, {
