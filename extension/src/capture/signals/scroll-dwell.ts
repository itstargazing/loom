...
import type { DwellSection } from "../types";
...
interface TrackedSection {
  ...
  excerpt: string;
  dwellMs: number;
  /** Currently intersecting the viewport at or above VISIBLE_RATIO. */
  onScreen: boolean;
  /** Timestamp accumulation started, or null when paused/off-screen. */
  since: number | null;
}

function qualifies(element: Element): boolean {
  const length = (element.textContent ?? "").trim().length;
  // Headings are short by nature but still worth tracking as section markers.
  if (/^H[1-4]$/.test(element.tagName)) return length >= 3;
 ...
 */
export function installScrollDwellSignal(dwellThresholdMs: number): () => void {
  const sections: TrackedSection[] = [];
  const byElement = new WeakMap<Element, TrackedSection>();
  let paused = document.visibilityState === "hidden";
  let emitted = false;

  const startAccruing = (section: TrackedSection, now: number): void => {
    if (section.since === null) section.since = now;
  };

  const stopAccruing = (section: TrackedSection, now: number): void => {
    if (section.since === null) return;
  ...
  const track = (elements: Iterable<Element>): void => {
    for (const element of elements) {
      if (sections.length >= MAX_SECTIONS) return;
      if (byElement.has(element) || !qualifies(element)) continue;
  ...
  const unwatchDom = watchDom((added) => {
    for (const element of added) {
      if (element.matches(SECTION_SELECTOR)) track([element]);
  ...
  const emitSummary = (): void => {
    ...
    const now = performance.now();
    for (const section of sections) stopAccruing(section, now);

    const read: DwellSection[] = sections
    ...
    captureEmitter.emit("scroll_dwell", {
      totalVisibleMs: Math.round(
        sections.reduce((total, section) => total + section.dwellMs, 0),
      ),
      sections: read,
