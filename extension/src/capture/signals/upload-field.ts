 ...
 */
export function installUploadFieldSignal(): () => void {
  const seen = new WeakSet<HTMLInputElement>();

  const report = (input: HTMLInputElement): void => {
    ...
    void chrome.runtime
      ...
      .then((response) => {
        if (!response?.ok || !response.match) return;
        ...
        showAutoAttachPrompt(
          ...
          async () => {
            ...
            if (!target || !response.match.hasFile) return;
            const fileResponse = await chrome.runtime.sendMessage({
              type: "auto-attach:fetch-file",
              documentId: response.match.documentId,
            });
            if (!fileResponse?.ok || !fileResponse.bytes) return;
            const bytes = fileResponse.bytes as number[];
            const file = new File(
              [new Uint8Array(bytes)],
              String(fileResponse.filename ?? response.match.filename),
              { type: String(fileResponse.mimeType ?? "application/octet-stream") },
            );
  ...
  const scan = (elements: Iterable<Element>): void => {
    for (const element of elements) {
      if (element instanceof HTMLInputElement) report(element);
    }
  };
  ...
  const unwatchDom = watchDom((added) => {
    for (const element of added) {
      if (element.matches(UPLOAD_SELECTOR)) scan([element]);
