 ...
 */
export function extractContext(range, limit = 600) {
    if (!range)
        return "";
...
export function labelTextFor(input) {
    if (input.labels && input.labels.length > 0) {
        const text = collapseWhitespace([...input.labels].map((label) => label.textContent ?? "").join(" "));
        if (text)
            return truncate(text, 200);
    }
    const aria = input.getAttribute("aria-label");
    if (aria)
        return truncate(collapseWhitespace(aria), 200);
    ...
    if (legend)
        return truncate(legend, 200);
    return "";
}
/** Broader text near an element, for matching against recently seen documents. */
export function surroundingTextFor(element, limit = 400) {
    const container = element.closest("form, fieldset, section, article, li, td") ??
